...
# set 22 as default SSH port
#if [ ${#_BORG_PORT} -eq 0 ]; then
#    _BORG_PORT="22"
#fi

# set ed25519 as default SSH port
if [ ${#_BORG_SSH_ENCRYTION} -eq 0 ]; then
    _BORG_SSH_ENCRYTION="ed25519"
fi

# set none as default Borrg encryption
if [ ${#_BORG_ENCRYPTION} -eq 0 ]; then
    _BORG_SSH_ENCRYTION="none"
fi
...
function set_borg_repo_path {
    ...
    if [ ${#_BORG_REPO_URL} -gt 0 ]; then
        BORG_REPO="ssh://${_BORG_REPO_URL}"
        bashio::log.debug "BORG_REPO set"
    else 
        BORG_REPO="ssh://${_BORG_USER}@${_BORG_HOST}:${_BORG_PORT}${_BORG_REPONAME}"
    fi
    bashio::log.debug "BORG_REPO set"
...
function init_borg_repo {
    if ! bashio::fs.directory_exists "${BORG_BASE_DIR}/.config/borg/security"; then
        ...
        borg init --encryption=${_BORG_ENCRYPTION} --debug ${BORG_REPO}
    fi
}
...
#    borg create ${_BORG_DEBUG} --compression ${_BORG_COMPRESSION} --stats ssh://${BORG_REPO}::${_BORG_ARCHIVE} ${_BORG_PATH}
    borg create ${_BORG_DEBUG} --compression ${_BORG_COMPRESSION} --stats -s --progress ${BORG_REPO}::"${BACKUP_TIME}" ${_BORG_PATH}
